...
$sqlDefEmp = "SELECT * FROM settings WHERE type='defaultEmployee' ORDER BY id ASC LIMIT 1";
$resultDefEmp = $conn->query($sqlDefEmp);

$sqlDefMan = "SELECT * FROM users_manager ORDER BY id ASC LIMIT 1";
$resultDefMan = $conn->query($sqlDefMan);

if ($resultDefEmp->num_rows > 0) {
    $rowDefEmp = $resultDefEmp->fetch_assoc();
    $defaultEmployee = $rowDefEmp['value'];
}else if($resultDefMan->num_rows > 0){
    $rowDefMan = $resultDefMan->fetch_assoc();
    $defaultEmployee = $rowDefMan['name'];
}else{
    $defaultEmployee = "";
}
        ...
        <form id="projectForm" method="post">
            <div class="panel grid grid-rows gap-4 px-6 pt-6">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="visibility" class="form-select text-white-dark">
                            <option selected value="user-client">Cari ve Görevli</option>
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <p>Görevliler</p>
                        <input name="employee" required tyoe="text" class="form-input text-white-dark" value="<?php echo $defaultEmployee ?>" readonly/>
                    </div>
                </div>
...
<!-- Project Add Modal -->
<script src="pages/client/scrumboard/modules/projectAdd.js"></script>
