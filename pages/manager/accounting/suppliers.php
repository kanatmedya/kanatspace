...
echo'<script>
    document.title = "Tedarikçi Carileri | '.$companyName.'";
</script>';

//Tablodaki Bilgiler İçin Sorgu
$sqlTotal = "SELECT id FROM users_client WHERE accountType = 'supplier'";
...
$countSql = "SELECT COUNT(*) AS total FROM users_client WHERE accountType = 'supplier'";
...
WHERE 
    uc.accountType = 'supplier'
GROUP BY 
    uc.id, uc.username, uc.accountType, uc.profilePicture, uc.name
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <h1 style="font-size:24px;margin-right:20px">Tedarikçi Carileri</h1><br>
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<td>
                                                    <div class='flex items-center gap-2'>
                                                        ...
                                                        <div>
                                                            <a href='supplier?id={$row['id']}  ' class='font-semibold hover:underline line-clamp-2 whitespace-normal'>
                                                                {$row['username']}
                                                            </a>
                                                        </div>
                                                    </div>
