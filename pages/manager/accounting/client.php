...
// SQL sorgusu
$query = "SELECT * FROM invoices WHERE clientID = ? AND (status = 'onOrder' OR status = 'completed') AND type = 'sale' ORDER BY date DESC"; // Client ID'ye göre sorgulama yapılıyor

// Client ID'yi hazırlayıp sorguyu çalıştırın
if ($stmt = $conn->prepare($query)) {
    $stmt->bind_param('i', $clientId);
    $stmt->execute();
    $resTab = $stmt->get_result(); // Sonuçları alıyoruz

    if ($resTab->num_rows > 0) {
        // Tablo satırlarını listeleyin
        while ($rowTab = $resTab->fetch_array(MYSQLI_ASSOC)) {
        
            if($rowTab['status']=='completed'){
                $style = "success";
            }else if($rowTab['status']=='rejected'){
                $style = "danger";
            }else{
                $style = "primary";
            }
            ?>
                <tr>
                    <!--<td>#<?php echo $rowTab['id'] ?></a></td>-->
                    <td><a href="invoice?id=<?php echo $rowTab['id'] ?>"><?php echo dateTR($rowTab['date']).'<br>'. dayTR($rowTab['date']); ?></a></td>
                    <td><a href="invoice?id=<?php echo $rowTab['id'] ?>" class="font-semibold"><?php echo $rowTab['title'] ?></a></td>
                    <td><?php echo translateType($rowTab['type']) ?></td>
                    <td><span class="badge badge-outline-<?php echo $style ?>"><?php echo translateStatus($rowTab['status']) ?></span></td>
                    <td><?php echo number_format($rowTab['amount'], 2, ',', '.') ?></td>
                    <td>
                        <div class="flex gap-4 items-center">
                        <span class="badge badge-outline-success">PDF</span>
                        <span class="badge badge-outline-primary">Link</span>
                            <!--<a href="apps-invoice-edit.html" class="hover:text-info">Düzenle</a>
                            <button type="button" class="hover:text-danger" @click="deleteRow(<?php echo $rowTab['id']; ?>)">Sil</button>-->
                        </div>
                    </td>
                </tr>
            <?php }
...
// SQL sorgusu
// Prepared statement ile sorgu
$queryPayments = "SELECT * FROM accounting_transactions WHERE sender = ? OR reciever = ? ORDER BY date DESC";

// Sorguyu hazırlayın
if ($stmtPayments = $conn->prepare($queryPayments)) {
    // Parametreleri bağlayın (bind)
    $stmtPayments->bind_param("ss", $clientUsername, $clientUsername); // "ss" iki string parametreyi temsil eder

    // Sorguyu çalıştırın
    $stmtPayments->execute();

    // Sonuçları alın
    $resTabPayments = $stmtPayments->get_result();
