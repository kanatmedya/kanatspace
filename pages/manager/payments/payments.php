...
// Toplam sonuç sayısını almak için sorgu
$sqlTotal = "SELECT * FROM accounting_recurring_transactions ORDER BY id";
$resultTotal = $conn->query($sqlTotal);
$totalResults = $resultTotal->num_rows;

// Toplam sonuç sayısını almak için sorgu
$sqlActive = "SELECT * FROM accounting_recurring_transactions WHERE payment_status = 0 ORDER BY id";
$resultActive = $conn->query($sqlActive);
$activeResults = $resultActive->num_rows;

// Toplam sonuç sayısını almak için sorgu
$sqlCompleted = "SELECT * FROM accounting_recurring_transactions WHERE payment_status = 1 ORDER BY id";
$resultCompleted = $conn->query($sqlCompleted);
$completedResults = $resultCompleted->num_rows;

// Toplam sonuç sayısını almak için sorgu
$sqlPass = "SELECT * FROM accounting_recurring_transactions WHERE payment_status = 0 AND due_date < NOW() ORDER BY id";
$resultPass = $conn->query($sqlPass);
$resultPass = $resultPass->num_rows;
...
// Sayfalı veri almak için sorgu
$sql = "SELECT * FROM accounting_recurring_transactions WHERE payment_status = 0 ORDER BY due_date 
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <h1 style="font-size:24px;margin-right:20px">Ödemeler</h1><br>
    ...
    <div class="mb-6 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    ...
                    <h5 class="text-xs text-[#506690]">Toplam Ödeme</h5>
                </div>
            </div>
        </div>
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $resultPass; ?></p>
                    <h5 class="text-xs text-[#506690]">Geciken Ödeme</h5>
                </div>
            </div>
        </div>
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $activeResults; ?></p>
                    <h5 class="text-xs text-[#506690]">Aktif Ödeme</h5>
                </div>
            </div>
        </div>
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $completedResults; ?></p>
                    <h5 class="text-xs text-[#506690]">Tamamlanan Ödeme</h5>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="panel flex-1 overflow-y-auto">
                <div class="mb-5 sm:absolute sm:top-5 ltr:sm:left-5 rtl:sm:right-5">
                    <div class="relative flex items-center justify-between gap-4">
                        <a href="paymentAdd" class="btn btn-primary gap-2 px-3">
                        ...
                        <div class="dataTable-container">
                            <table id="myTable" class="whitespace-nowrap dataTable-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-sortable="" style="width: 13%;">
                                            <a href="#" class="dataTable-sorter">Tarih</a>
                                        </th>
                                        <th data-sortable="" style="width: 34.7856%;">
                                            <a href="#" class="dataTable-sorter">Gönderici -> Alıcı</a>
                                        </th>
                                        <th data-sortable="" style="width: 10.5118%;">
                                            <a href="#" class="dataTable-sorter">İşlem Türü</a>
                                        </th>
                                        <th data-sortable="" style="width: 10.0968%;">
                                            <a href="#" class="dataTable-sorter">Tutar</a>
                                        </th>
                                ...
                                <tbody>
                                    ...
                                    function calculateDueDate($created_count, $installment_count, $next_due_date, $recurrence_period) {
                                        // Eğer recurrence_period 'taksit' ise sonsuz işareti döndür
                                        if ($recurrence_period != 'taksit') {
                                            return '∞';
                                        }else{
                                    
                                            // Eklenmesi gereken süreyi hesapla
                                            $addDate = $installment_count - $created_count;
                                        
                                            // next_due_date'i DateTime objesine çevir
                                            $date = new DateTime($next_due_date);
                                        
                                            // Aylar Türkçe olacak şekilde diziyi tanımla
                                            $months = array(
                                                'January' => 'Ocak',
                                                'February' => 'Şubat',
                                                'March' => 'Mart',
                                            ...
                                            // Süreyi recurrence_period'a göre ekle
                                            $date->modify("+$addDate months");
                                        
                                            // Türkçe ay ismini almak için ayı değiştirme
                                    ...
                                    function formatCurrency($value,$currency) {
                                        if (is_numeric($value)) {
                                            ...
                                            return $value; // Sayı değilse orijinal değeri döner
                                        }
                                    }
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            $paymentDate = $row['due_date']; // Örneğin: "2024-04-28" formatında
                                            
                                            // Tarih formatını strtotime ile zaman damgasına çeviriyoruz
                                            $timestamp = strtotime($paymentDate);
                                            
                                            // Ay adını Türkçe'ye çevirmek için İngilizce ay ismini alıyoruz
                                            $englishMonth = date('F', $timestamp); // Ay ismi örneğin "April" olur
                                            
                                            // Ay adını Türkçe karşılığı ile değiştiriyoruz
                                            $turkishMonth = $months[$englishMonth];
                                            
                                            // Tarihi istediğiniz formata göre düzenliyoruz: "28 Nisan 2024"
                                            $paymentDate = date('d', $timestamp) . ' ' . $turkishMonth . ' ' . date('Y', $timestamp);

                                            $scheduledID = $row['transaction_id'];
                                            
                                            $payment = $row['installment_amount'];
                                            $payment = formatCurrency($payment,'₺');
                                            
                                            // Veritabanı sorgusu
                                            $sqlScheduled = "SELECT * FROM accounting_scheduled_payments WHERE id = $scheduledID";
                                            $resultScheduled = $conn->query($sqlScheduled);
                                        
                                            // Sonuçları kontrol et
                                            if ($resultScheduled->num_rows > 0) {
                                                // Veritabanından gelen veriyi yazdır
                                                while ($rowScheduled = $resultScheduled->fetch_assoc()) {
                                                    $sender = $rowScheduled['sender'];
                                                    $reciever = $rowScheduled['reciever'];
                                                    $passCount = $rowScheduled['created_count'];
                                                    $totalCount = $rowScheduled['repeat_count'];
                                                    $leftCount = $totalCount-$passCount;
                                                    $type = $rowScheduled['type'];
                                                    if($type==0){
                                                        $type = 'Gider';
                                                    }else if($type==1){
                                                        $type = 'Gelir';
                                                ...
                                                echo "ID: $id için sonuç bulunamadı.<br>";
                                            }
                                            
                                            $arama = $reciever . ' ' . $sender . ' ' . $paymentDate;

                                            echo "<tr class='product-row' data-product-name='{$arama}'>";
                                            echo "<td><input type='checkbox' class='form-checkbox mt-1' id='chk{$row['id']}' value='{$row['id']}'></td>";
                                            echo "<td><div class='font-semibold'>{$paymentDate}</div></td>";
                                            echo "<td>
                                                    <div class='flex items-center gap-2'>
                                                        <div>
                                                            <a href='payment?{$row['id']}' class='accountText font-semibold text-primary line-clamp-1 whitespace-normal'>
                                                                {$sender}
                                                            </a>
                                                        </div>
                                                        <p> -> </p>
                                            ...
                                            echo "<td><div class='font-semibold'>{$type}</div></td>";
                                            echo "<td><div class='font-semibold'>{$payment}</div></td>";
                                            echo "<td>
                                                    <div class='flex gap-4 items-center'>
                                                        <a href='paymentUpdate?{$row['id']}' class='hover:text-info'>
                                                        ...
                                                        <a href='payment?{$row['id']}' class='hover:text-primary' >
                            ...
                            <div class="dataTable-dropdown">
                                <label><span class="ml-2">
                                        <select class="dataTable-selector" onchange="degis()">
                                            <option value="25" <?php $selectedPage= false; if ($itemsPerPage == 25) {echo 'selected'; $selectedPage=true;}?>>25</option>
                                            <option value="50" <?php if ($itemsPerPage == 50) {echo 'selected'; $selectedPage=true;}?>>50</option>
                                            <option value="100" <?php if ($itemsPerPage == 100) {echo 'selected'; $selectedPage=true;}?>>100</option>
                                            <option value="1000" <?php if ($itemsPerPage == 1000) {echo 'selected'; $selectedPage=true;}?>>1000</option>
                                            <?php if ($selectedPage == false){ echo '<option value="'.$itemsPerPage.'" selected>'.$itemsPerPage.'</option>';} ?>
                                        </select>
                                    </span></label>
                            </div>
                            <nav class="dataTable-pagination">
                                <ul class="dataTable-pagination-list">
                                    ...
                                    <?php for ($i = 1; $i <= ceil($totalResults / $itemsPerPage); $i++): ?>
                                        ...
                                        <?php else: ?>
                                            <li><a href="?page=<?= $i ?>&queue=<?= $itemsPerPage ?>"><?= $i ?></a></li>
                                        <?php endif; ?>
                                    <?php endfor; ?>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        updateLinks.forEach(link => {
            link.addEventListener('click', function (event) {
                ...
                window.location.href = 'paymentUpdate?' + productId;
            });
        });
    });
</script>
