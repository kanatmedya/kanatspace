        ...
        <div class="modal-content inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div class="mt-2 comment-input-container">
                    <textarea id="commentText" class="px-1 py-1 mt-1 block w-full border border-gray-300 rounded-md shadow-sm" rows="4"></textarea>
                    ...
                    <div class="attach-icon">
                        <button type="button" id="cancelUpload" class="btn-commentmodal btn btn-danger gap-2" style="display: none;">
                ...
                <div id="uploadProgress">
                    ...
                    <div id="uploadTimeRemaining" class="upload-time-remaining"></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function convertRichText(text) {
        // https:// veya http:// ile başlayanları bul ve <a> tag'ine çevir
        return text.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank">$1</a>');
    }


    $(document).ready(function() {
    var currentUser = '<?php echo $_SESSION["userID"]; ?>'; // PHP'den oturumdaki kullanıcı ismini al
    var xhr; // Global olarak xhr nesnesi tanımla
    ...
    function previewFiles(inputElement) {
        ...
        previewElement.empty(); // Önceki dosya önizlemelerini temizle
                    ...
                    comments.forEach(function(comment) {
                        ...
                        var commentHtml = `
                            <div class="comment ${commentClass}" data-comment-id="${comment.id}">
                                ...
                                <div class="text" style="text-align: left">${convertRichText(commentValue).replace(/\n/g, '<br>')}</div>
    ...
    function saveComment() {
        ...
        if (totalSize > 0) {
            $('#uploadProgress').show();
            $('#uploadStatus').text('Yükleniyor: 0% (0 / ' + (totalSize / (1024 * 1024)).toFixed(2) + ' MB)');
            $('#uploadTimeRemaining').text('Tahmini Kalan Süre: Hesaplanıyor...');
            $('#cancelUpload').show();
        }
        ...
        xhr = $.ajax({
            ...
            xhr: function() {
                ...
                xhr.upload.addEventListener('progress', function(e) {
                    if (e.lengthComputable) {
                        ...
                        var uploadSpeed = uploadedSize / elapsedTime; // bytes per second
                        var remainingTime = (totalSize - uploadedSize) / uploadSpeed; // seconds

                        if (totalSize > 0) {
                            $('#uploadStatus').text('Yükleniyor: ' + percentComplete.toFixed(2) + '% (' + (uploadedSize / (1024 * 1024)).toFixed(2) + ' / ' + (totalSize / (1024 * 1024)).toFixed(2) + ' MB)');
                            $('#uploadTimeRemaining').text('Tahmini Kalan Süre: ' + Math.ceil(remainingTime) + ' saniye');
                        }
                    }
                }, false);
                return xhr;
                ...
                if (response.status === 'success') {
                    ...
                    var commentHtml = `
                        <div class="comment ${commentClass}" data-comment-id="${response.commentId}">
                            ...
                            <div class="text">${commentText.replace(/\n/g, '<br>')}</div>
            ...
            error: function(xhr, status, error) {
                if (status !== 'abort') { // İptal edilen istekler için hata mesajı gösterme
    ...
    // Yükleme işlemini iptal etmek için
    $('#cancelUpload').click(function() {
        if (xhr) {
            xhr.abort();
            $('#uploadStatus').text('Yükleme iptal edildi.');
            $('#uploadTimeRemaining').text('');
            $('#uploadProgress').hide();
            $('#cancelUpload').hide();
        }
    ...
    // Yorum silmek için
    $('#commentContainer').on('click', '.delete-icon', function() {
