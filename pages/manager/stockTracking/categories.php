...
$sql = "SELECT id,type, categoryName, createDate, updateDate FROM products_category  WHERE type='main'";
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
            ...
            <div class="panel flex-1 overflow-y-auto">
                <div class="mb-5 sm:absolute sm:top-5 ltr:sm:left-5 rtl:sm:right-5">
                    <div class="relative flex items-center justify-between gap-4">
                        <a href="categoryAdd" class="btn btn-primary gap-2 px-3">
  ...
  while ($row = $result->fetch_assoc()) {
    echo '<tr>';
    echo '<td><input type="checkbox" class="form-checkbox mt-1" :id="\'chk\' + 1" :value="(1)" x-model.number="selectedRows" id="chk1" value="1"></td>';
    echo '<td>
            <div class="flex items-center gap-2">
              <div class="p-px bg-white-dark/30 rounded w-max"><img class="h-8 w-8 rounded object-cover" src="assets/images/product/product-1.jpg"></div>
              <div><a href="apps-ecommerce-product-details.html" class="font-semibold text-primary hover:underline line-clamp-1 whitespace-normal">' . htmlspecialchars($row['categoryName']) . '</a>
                <div><span class="font-semibold"></span><span>' . ($row['type']=="main" ? "Ana Kategori" : "Alt Kategori" . ''). '</span></div>
              </div>
            </div>
          </td>';
              echo '<td><div class="font-semibold">25000</div></td>'; // Benzer şekilde diğer sütunlar için de aynı işlemi yapabilirsiniz
echo '<td><div class="font-semibold">' . ($row['createDate'] ? htmlspecialchars($row['createDate']) : '') . '</div></td>';
        echo '<td><div class="font-semibold">' . ($row['updateDate'] ? htmlspecialchars($row['updateDate']) : '') . '</div></td>';
echo "<td>
                            <div class='flex gap-4 items-center'>
                                <a href='categoryUpdate.php?id={$row['id']}'>
                                ...
                                <a href='apps-ecommerce-product-details.html' class='hover:text-primary'>
    ...
    function showAlert(id) {
        new window.Swal({
            ...
            if (result.value) {
                ...
                xhr.open('POST', 'pages/manager/stockTracking/modules/delete_category.php', true);
