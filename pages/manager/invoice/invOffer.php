<?php
    $sqlTab = "SELECT * FROM invoices 
WHERE status IN ('onOffer', 'rejected')
ORDER BY 
    CASE 
        WHEN status = 'onOffer' THEN 1 
...
";
    ...
    $sqlTotalSum = "SELECT SUM(amount) as amount FROM invoices WHERE status = 'onOffer'";
    $resTotalSum = $conn->query($sqlTotalSum);
    $rowTotalSum = $resTotalSum->fetch_array();
    if($rowTotalSum["amount"] != null){
        $totalSum = number_format($rowTotalSum['amount'], 2, ',', '.');
    }else{
        $totalSum = 0;
    }

    $sqlTotalSumSale = "SELECT SUM(amount) as amount FROM invoices WHERE status = 'onOffer' and type='sale'";
    $resTotalSumSale = $conn->query($sqlTotalSumSale);
    $rowTotalSumSale = $resTotalSumSale->fetch_array();
    if($rowTotalSumSale["amount"] != null){
        $totalSumSale = number_format($rowTotalSumSale['amount'], 2, ',', '.');
    }else{
        $totalSumSale = 0;
    }

    $sqlTotalSumPurc = "SELECT SUM(amount) as amount FROM invoices WHERE status = 'onOffer' and type='purchase'";
    $resTotalSumPurc = $conn->query($sqlTotalSumPurc);
    $rowTotalSumPurc = $resTotalSumPurc->fetch_array();
    if($rowTotalSumPurc["amount"] != null){
        $totalSumPurc = number_format($rowTotalSumPurc['amount'], 2, ',', '.');
    }else{
        $totalSumPurc = 0;
    }
?>
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <h1 style="font-size:24px;margin-right:20px">Teklifler</h1><br>
    <div class="mb-6 grid gap-6 sm:grid-cols-3 xl:grid-cols-4">
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $totalNum?></p>
                    <h5 class="text-xs text-[#506690]">Toplam Teklif Sayısı</h5>
                </div>
            </div>
        </div>
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    ...
                    <h5 class="text-xs text-[#506690]">Toplam Teklif Tutarı</h5>
                </div>
            </div>
        </div>
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $totalSumPurc ?></p>
                    <h5 class="text-xs text-[#506690]">Alış Teklifleri</h5>
                </div>
            </div>
        </div>
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $totalSumSale?></p>
                    <h5 class="text-xs text-[#506690]">Satış Teklifleri</h5>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
                    ...
                    <div class="dataTable-container">
                        <table id="myTable" class="whitespace-nowrap dataTable-table">
                            <thead>
                                <tr>
                                    <th data-sortable="false" style="width: 5%;"><input type="checkbox" class="form-checkbox" :checked="checkAllCheckbox" :value="checkAllCheckbox" @change="checkAll($event.target.checked)"></th>
                                    <th data-sortable="" style="width: 15%;"><a href="#" class="dataTable-sorter">Müşteri</a></th>
                                    <th data-sortable="" style="width: 25%;"><a href="#" class="dataTable-sorter">Teklif Başlığı</a></th>
                                    <th data-sortable="" style="width: 10%;"><a href="#" class="dataTable-sorter">Tutar</a></th>
                                    <th>Durumu</th>
                                    <th data-sortable="" style="width: 10%;"><a href="#" class="dataTable-sorter">Tarih</a></th>
                                    <th data-sortable="" style="width: 10;"><a href="#" class="dataTable-sorter">Belge</a></th>
                                    <th data-sortable="false" style="width: 10%;">Eylemler</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                    while($rowTab = $resTab->fetch_array()){
                                        include "row.php";
                                    }
                                ?>
                            </tbody>
                    ...
                    <div class="dataTable-bottom">
                        ...
                        <div class="dataTable-dropdown"><label><span class="ml-2"><select class="dataTable-selector">
                        ...
                        <nav class="dataTable-pagination">
                            <ul class="dataTable-pagination-list">
                                <li class="pager"><a href="#" data-page="1"><svg width="24" height="24"
                                ...
                                <li class="pager"><a href="#" data-page="2"><svg width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                                            class="w-4.5 h-4.5 rtl:rotate-180">
...
$(document).ready(function() {
    $(".status-dropdown").change(function() {
        ...
        $.ajax({
            url: "php/ajax-update-invoice-status.php",
            type: "POST",
