<?php
    ...
    $sqlTab = "SELECT * FROM invoices WHERE type = 'sale' AND status='completed' ORDER BY id DESC";
    ...
    $sqlTotalSum = "SELECT SUM(amount) as amount FROM invoices WHERE type = 'sale' AND status='completed'";
    $resTotalSum = $conn->query($sqlTotalSum);
    $rowTotalSum = $resTotalSum->fetch_array();
    $totalSum = number_format($rowTotalSum['amount'], 2, ',', '.');
    
    //Tamamlanan Satış Faturalarının Toplam Tutarını Hesaplıyor
    $sqlCollectingSum = "SELECT SUM(amount) as amount FROM accounting_transactions WHERE categorySub = '2' ";
    $resCollectingSum = $conn->query($sqlCollectingSum);
    $rowCollectingSum = $resCollectingSum->fetch_array();
    $totalCollectingSum = number_format($rowCollectingSum['amount'], 2, ',', '.');

    //Yapılan Ödemeleri Hesaplıyor
    $sqlSumPayment = "SELECT SUM(amount) as amount FROM accounting_transactions WHERE categorySub ='2'";
...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <h1 style="font-size:24px;margin-right:20px">Satış Faturaları</h1><br>
    <div class="mb-6 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $sumPayment?></p>
                    <h5 class="text-xs text-[#506690]">Yapılan Tahsilat</h5>
                </div>
            </div>
        </div>
        ...
        <div class="panel h-full p-0">
            <div class="flex p-5">
                ...
                <div class="font-semibold ltr:ml-3 rtl:mr-3">
                    <p class="text-xl dark:text-white-light"><?php echo $sumRemain?></p>
                    <h5 class="text-xs text-[#506690]">Kalan</h5>
                </div>
            </div>
        </div>
    </div>
