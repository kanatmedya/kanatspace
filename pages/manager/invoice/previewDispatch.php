...
if (isset($_GET['id'])) {
    $invID = $_GET['id'];
    $sqlInv = "SELECT * FROM dispatch WHERE id = '$invID'";
} elseif (isset($_GET['key'])) {
    $invKey = $_GET['key'];
    $sqlInv = "SELECT * FROM dispatch WHERE secretKey = '$invKey'";
}
            ...
            <div class="flex flex-col flex-wrap justify-between gap-6 lg:flex-row">
                <div class="flex-1">
                    <div class="space-y-1 text-white-dark">
                        ...
                        <div>
                            <?php echo $rowCli['addressDistrict'] . '/' . $rowCli['addressProvince'] . '/' . $rowCli['addressCountry']; ?>
                        </div>
                    </div>
                </div>
                <div class="flex flex-col justify-between gap-6 sm:flex-row lg:w-2/3">
                    <div class="xl:1/3 sm:w-1/2 lg:w-2/5">
                        ...
                        <div class="mb-2 flex w-full items-center justify-between">
                            ...
                            <div><?php echo $rowCli['contact']; ?></div>
                        </div>
                        <div class="flex w-full items-center justify-between">
                            <div class="text-white-dark">Telefon :</div>
                            <div><?php echo $rowCli['contactPhone']; ?></div>
                        </div>
                    </div>
                    <div class="xl:1/3 sm:w-1/2 lg:w-2/5">
                        <div class="mb-2 flex w-full items-center justify-between">
                        ...
                        <div class="flex w-full items-center justify-between">
                            <div class="text-white-dark">Son Ödeme Tarihi :</div>
                            <div><?php
                ...
                <table class="table-striped">
                    ...
                    <tbody>
                        ...
                        while ($rowInvProducts = $resInvProducts->fetch_array()) {
                            ...
                            // Ürün Bilgilerini ID ile Çekiyor
                            $productID = $rowInvProducts['productID'];
                            $sqlProduct = "SELECT * FROM products WHERE id = '$productID'";
                            $resProduct = $conn->query($sqlProduct);
                            $rowProduct = $resProduct->fetch_array();

                            // Normal Toplam Fiyat
                            $cartMainTotal += $rowInvProducts['quantity'] * $rowProduct['price'];

                            // İndirimli Toplam Fiyat
                            $cartTotal += $rowInvProducts['quantity'] * $rowInvProducts['price'];

                            // Vergi Toplam
                            if ($rowProduct['tax'] == 0) {
                                $tax = 0;
                            } else {
                                $tax = ($rowInvProducts['quantity'] * $rowInvProducts['price']) / 100 * $rowInvProducts['tax'];
                            }

                            $taxTotal += $tax;

                            // Toplam
                            $mainTotal += ($rowInvProducts['quantity'] * $rowInvProducts['price']) + ($tax);

                            // İndirim Toplam
                            $discount = $rowProduct['price'] - $rowInvProducts['price'];
                            $discountTotal += ($rowInvProducts['quantity'] * $rowProduct['price']) - ($rowInvProducts['quantity'] * $rowInvProducts['price']);

                            echo '
                        <tr>
                            <td>' . $sNo . '</td>
                            <td>' . $rowProduct['name'] . '</td>
                            <td>' . $rowInvProducts['quantity'] . '</td>
                            <td class="ltr:text-right rtl:text-left">' . $rowProduct['price'] . '</td>
                            <td class="ltr:text-right rtl:text-left">' . $discount . '</td>
        ...
        <div class="mt-6 grid grid-cols-1 px-4 sm:grid-cols-2">
            ...
            <div class="space-y-2 ltr:text-right rtl:text-left">
                <div class="flex items-center">
                    ...
                    <div class="w-[37%]"><?php echo number_format($cartMainTotal, 2, ',', '.') ?></div>
                </div>
                <div class="flex items-center">
                    <div class="flex-1">İskonto</div>
                    <div class="w-[37%]"><?php echo number_format($discountTotal, 2, ',', '.') ?></div>
                </div>
                <div class="flex items-center">
                    <div class="flex-1">Toplam</div>
                    <div class="w-[37%]"><?php echo number_format($cartTotal, 2, ',', '.') ?></div>
                </div>
                <div class="flex items-center">
                    <div class="flex-1">Vergi</div>
                    <div class="w-[37%]"><?php echo number_format($taxTotal, 2, ',', '.') ?></div>
                </div>
                <div class="flex items-center text-lg font-semibold">
                    <div class="flex-1">Genel Toplam</div>
                    <div class="w-[37%]"><?php echo number_format($mainTotal, 2, ',', '.') ?></div>
                </div>
            </div>
        </div>
    </div>
